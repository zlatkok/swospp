...
char *int2str(int num)
{
    ...
    if (num < 0) {
        do {
            num /= 10;
            *p-- = '0' + num * 10 - oldNum;    /* avoid having to do moduo */
            oldNum = num;
        } while (num);
        *p = '-';
    } else {
        do {
            num /= 10;
            *p-- = '0' + oldNum - num * 10;    /* avoid having to do moduo */
            oldNum = num;
        } while (num);
        p++;
    }
...
/** strol
    ...
    base   -  conversion base, if 0 it will be determinded by first characters
